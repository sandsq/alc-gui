...
fn write_toml(filename: &str, layout_info: LayoutInfoTomlAdapter, num_threads: usize, genetic_options: GeneticOptions, keycode_options: KeycodeOptions, dataset_options: DatasetOptions, score_options: ScoreOptions) -> Result<(), AlcError> {
	...
	let layout_optimizer_config = LayoutOptimizerConfig {
		...
		valid_keycodes: vec![], //generate_default_keycode_set(&keycode_options).into_iter().collect(),
		dataset_options,
		score_options,
		num_threads,
	};
	let toml_adapter = LayoutOptimizerTomlAdapter {
		layout_info,
		layout_optimizer_config,
	};
	toml_adapter.write_to_file(filename)?;
	// let toml = format!("\
	// [layout_info]\n\
	// num_rows = {}\n\
	// num_cols = {}\n\
...
#[tauri::command]
fn check_saved_toml(filename: String) -> bool {
	println!("checking existence of {}", filename);
