	...
	function fill_phalanx_layer_from_string(phalanx_string) {
		...
		for (let i = 0; i < rows.length; i++) {
			...
			if (phalanx_layer[0].length != cols.length) {
				alert(
					`the number of columns in the phalanx layer (${phalanx_layer[0].length}) does not match the number of columns found from the config (${cols.length}: ${cols}); this is probably a developer error due to parsing the config incorrectly`
				);
				return;
			}
