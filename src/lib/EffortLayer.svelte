<script>
	...
	function fill_effort_layer_from_string(effort_string) {
		...
		if (effort_layer.length != rows.length) {
			alert(
				`the number of rows in the effort layer (${effort_layer[0].length}) does not match the number of rows found from the config (${rows.length}: ${rows}); this is probably a developer error due to parsing the config incorrectly`
			);
			return;
		}
		for (let i = 0; i < rows.length; i++) {
			...
			if (effort_layer[0].length != cols.length) {
				alert(
					`the number of columns in the effort layer (${effort_layer[0].length}) does not match the number of columns found from the config (${cols.length}: ${cols}); this is probably a developer error due to parsing the config incorrectly`
				);
				return;
			}
